...
impl TenstorrentMockGenerator {
    ...
    fn add_temperature_sensors(&self, template: &mut String, gpus: &[GpuMetrics]) {
        ...
        for (sensor_name, description) in sensors {
            ...
            template.push_str(&format!("# TYPE all_smi_temperature_{sensor_name}_celsius gauge\n"));
    ...
    pub fn render_tenstorrent_response(&self, template: &str, gpus: &[GpuMetrics]) -> String {
        ...
        for (i, gpu) in gpus.iter().enumerate() {
            ...
            response = response.replace(&format!("{{{{SOC_UTIL_{i}}}}}"), &format!("{:.2}", soc_util));
            
            // Temperature sensors (variations from main temp)
            let asic_temp = gpu.temperature_celsius;
            let vreg_temp = (asic_temp as f32 + rng.random_range(-5.0..5.0)).clamp(30.0, 90.0) as u32;
            let inlet_temp = (asic_temp as f32 - rng.random_range(10.0..20.0)).clamp(20.0, 60.0) as u32;
